...
def get_config():
    ...
    parser.add_argument("--wandb_usr", type=str, default='None', help="wandb user name")

    parser.add_argument("--actor_ps_mode", type=str, default='ps', choices=['ps', 'nps', 'lora', 'mtl', 'seps', 'sepslora'], help="parameter sharing mode for actor. Determines what model architecture used")
    parser.add_argument("--critic_ps_mode", type=str, default='ps', choices=['ps', 'nps'], help="parameter sharing mode for critic. Determines what model architecture used")
    
    parser.add_argument("--lora_config", nargs='*', type=int, default=[1,1,1,1,1], help="list of int of 1 and 0 indicating which layer to use lora. layers are (mlp_base(2), gru(2), act(1))")
    parser.add_argument("--r", type=int, default=4, help="rank r for LoRA.")
    parser.add_argument("--lora_alpha", type=float, default=1, help="the coefficient for scaling the Wd, i.e. W = W0 + lora_alpha/r * Wd")
    parser.add_argument("--train_w0", type=int, default=0, help="whether to train W0 of lora")
    parser.add_argument("--train_lora", type=int, default=1, help="whether to train AB of lora")
    parser.add_argument("--train_lora_bias", type=int, default=1, help="whether to train agent specific bias")
    
    parser.add_argument("--init_W0_scale", type=float, default=1, help="the alpha coefficient that controls the scale of W0 of lora; scale of Wd would be 1-alpha")
    parser.add_argument("--sg_warmup", type=float, default=0, help="the warmup percentage of total episodes for sg")
    parser.add_argument("--sg_cooldown", type=float, default=0, help="the cooldown percentage of total episodes for sg")
    parser.add_argument("--sg2_period", type=int, default=1, help="the period for periodic switch between train W0 and train dW (sg_version=2)")
    parser.add_argument("--sg2_W0_first", type=int, default=1, help="train W0 first or dW first: 1 for W0 first, 0 for dW first (sg_version=2)")

    parser.add_argument("--train_W0_ab", nargs='*', type=float, default=[1, 0], help="Config of a, b when training W0. [a, b] indicates W0 scale and Wd scale of lora")
    parser.add_argument("--train_Wd_ab", nargs='*', type=float, default=[0, 1], help="Config of a, b when training Wd. [a, b] indicates W0 scale and Wd scale of lora")

    parser.add_argument("--sg_version", type=int, default=0, help="version of grad scaling.")

    parser.add_argument("--step_shift", type=int, default=0, help="shift of logging step")

    parser.add_argument("--use_wandb", action='store_false', default=True, help="[for wandb usage], by default True, will log date to wandb server. or else will use tensorboard to log data.")
    parser.add_argument("--project_name", type=str, default="my_project", help="wandb project name.")

    parser.add_argument("--use_mappo", type=int, default=0, help="whether to use MAPPO: 0 for False; 1 for True. This flag overwrite hetero_actor")
    parser.add_argument("--gaussian_version", type=int, default=1, help="version of the gaussian distribution used in continuous action layer.")

    parser.add_argument("--group_id", type=str, default='None', help="For wandb figure grouping purpose")
    
    parser.add_argument("--obs_agent_id", type=args_str2bool, default=True, help="whether to add agent id into the observation")
    parser.add_argument("--state_agent_id", type=args_str2bool, default=True, help="whether to add agent id into the state")

    parser.add_argument("--mappo_epoch", type=int, default=0, help="number of epochs for mappo update. If 0, use the same epochs as ppo_epoch")
    parser.add_argument("--mappo_clip_param", type=float, default=0, help="clip param for mappo update. If 0, use the same epochs as clip_param")

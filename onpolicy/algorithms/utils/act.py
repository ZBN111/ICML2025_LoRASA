...
class ACTLayer(nn.Module):
    ...
    def __init__(self, action_space, inputs_dim, use_orthogonal, gain, args:Namespace=None):
        ...
        if action_space.__class__.__name__ == "Discrete":
            ...
            if args.gaussian_version == 1:
                ...
                self.action_out = DiagGaussian(inputs_dim, action_dim,
                                            use_orthogonal, gain, args.log_std_init, mu_out)
            elif args.gaussian_version == 2:
                from onpolicy.algorithms.utils.distributions import DiagGaussian2
                self.action_out = DiagGaussian2(inputs_dim, action_dim,
                                            use_orthogonal, gain, args)
            elif args.gaussian_version == 3:
                from onpolicy.algorithms.utils.distributions import DiagGaussian3
                self.action_out = DiagGaussian3(inputs_dim, action_dim,
                                            use_orthogonal, gain, args)
            else:
    ...
    def evaluate_actions(self,
        ...
        if self.mixed_action:
            ...
            action_logits = self.action_out(x, available_actions)
            if self.continuous_action:
                scaled_action = action / self.action_range
            else:
                scaled_action = action
            action_log_probs = action_logits.log_probs(scaled_action)
            if active_masks is not None:
                dist_entropy = (

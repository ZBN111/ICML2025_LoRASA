...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        if act_space.__class__.__name__ in [
            ...
            elif args.gaussian_version in [2,3]:
                self.action_log_probs = np.zeros(
    ...
    def compute_log_probs(self, policy):
        ...
        cur_action_log_probs = cur_action_log_probs.reshape(*self.action_log_probs.shape)
        return cur_action_log_probs
    ...
    def some_agent_compute_returns_from_current_policy(
        ...
        if self._share_policy:
            ...
            if policy.critic_ps_mode == 'loraq':
                next_values, _, _, _, _ = policy.get_actions(
                    np.concatenate(self.share_obs[-1]),
                    np.concatenate(self.obs[-1]),
                    np.concatenate(self.rnn_states[-1]),
                    np.concatenate(self.rnn_states_critic[-1]),
                    np.concatenate(self.masks[-1]),
                )
                next_value = next_values.reshape(-1, self.num_agents, *next_values.shape[1:])[:, agent_ids].reshape(-1, *next_values.shape[1:])
            else:
                next_value = policy.get_values(
                    np.concatenate(self.share_obs[-1][:, agent_ids]),
                    np.concatenate(self.rnn_states_critic[-1][:, agent_ids]),
    ...
    def compute_gaes_with_IS_ratios(self, ratios):
        gaes = np.zeros_like(self.deltas)
        gae = 0
        for step in reversed(range(self.rewards.shape[0])):
            if self._use_popart or self._use_valuenorm:
                # step + 1
                ...
                gae = (delta + self.gamma * self.gae_lambda * gae *
                        self.masks[step + 1] * ratios[step+1])
                if self._use_proper_time_limits:
                    gae = gae * self.bad_masks[step + 1]
                gaes[step] = gae   
        return gaes

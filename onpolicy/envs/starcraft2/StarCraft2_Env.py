...
class StarCraft2Env(MultiAgentEnv):
    ...
    def _launch(self):
        ...
        create = sc_pb.RequestCreateGame(
            local_map=sc_pb.LocalMap(map_path=_map.path,
    ...
    def get_agent_action(self, a_id, action):
        ...
        assert avail_actions[
            action] == 1, "Agent {} cannot perform action {}".format(
                a_id, action)
    ...
    def get_agent_action_heuristic(self, a_id, action):
        ...
        if self.heuristic_rest and self.get_avail_agent_actions(
                a_id)[action_num] == 0:
                ...
                if dist < sight_range and al_unit.health > 0:  # visible and alive
                    ...
                    if self.obs_all_health:
                        ...
                        if self.shield_bits_ally > 0:
                            ...
                            ally_feats[
                                i, ind] = al_unit.shield / max_shield  # shield
                            ind += 1
    ...
    def get_state(self, agent_id=-1):
        ...
        if self.obs_instead_of_state:
            obs_concat = np.concatenate(self.get_obs(),
        ...
        if (self.use_mustalive and unit.health > 0) or (
            ...
            for al_id, al_unit in self.agents.items():
                if al_unit.health > 0:
                    ...
                    ally_state[
                        al_id,
                        0] = al_unit.health / al_unit.health_max  # health
                    ...
                    if self.shield_bits_ally > 0:
                        max_shield = self.unit_max_shield(al_unit)
                        ally_state[al_id,
                                   ind] = al_unit.shield / max_shield  # shield
                        ind += 1
            ...
            for e_id, e_unit in self.enemies.items():
                if e_unit.health > 0:
                    ...
                    enemy_state[
                        e_id, 0] = e_unit.health / e_unit.health_max  # health
                    ...
                    if self.shield_bits_enemy > 0:
                        max_shield = self.unit_max_shield(e_unit)
                        enemy_state[e_id,
                                    ind] = e_unit.shield / max_shield  # shield
                        ind += 1
    ...
    def get_state_agent(self, agent_id):
        ...
        if (self.use_mustalive and unit.health > 0) or (
            ...
            for i, al_id in enumerate(al_ids):
                ...
                if al_unit.health > 0:  # visible and alive
                    ...
                    if self.map_type == "MMM" and al_unit.unit_type == self.medivac_id:
                        ...
                        ally_feats[
                            i,
                            4] = al_unit.weapon_cooldown / max_cd  # cooldown
            ...
            if self.unit_type_bits > 0:
                ...
                own_feats[ind + type_id] = 1
                ind += self.unit_type_bits
            # NOTE: the agent state also consist the last action of all agents
            if self.state_last_action:
                own_feats[ind:] = self.last_action[agent_id]
    ...
    def get_obs_size(self):
        ...
        return [
            all_feats *
            self.stacked_frames if self.use_stacked_frames else all_feats,
    ...
    def update_units(self):
        ...
        if n_ally_alive == 0 and n_enemy_alive > 0 or self.only_medivac_left(
                ally=True):
            return -1  # lost
